...
def preprocess_data_for_interpolation(collection):
    ...
    if len(filtered) == 0:
        ...
        if "Force2" in collection.columns:
            filtered = collection.ops.filter_by_value(
    ...
    if len(filtered) < 2:
        print(
            "警告: 補間には少なくとも2点以上のデータが必要です。フィルタリングを適用せずに続行します。"
        )
        return collection
...
def demonstrate_interpolation():
    ...
    try:
        ...
        result = selected_ops.interpolate_missing(
            method="linear", columns=["Force1", "Displacement1"]
        ).end()
    ...
    try:
        ...
        result = step_ops.interpolate(
            point_count=len(step_range) * 2, method="cubic"  # より細かい補間
        ).end()
    ...
    try:
        result = ops.interpolate(
            point_count=8, columns=["Force1", "Displacement1"]
        ).end()
        ...
        if len(original_force2) < len(result_force2):
            ...
            print(
                f"    補間後のForce2 (先頭のみ): {result_force2[:len(original_force2)]}"
            )
    except Exception as e:
