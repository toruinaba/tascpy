...
def create_sample_coordinate_data():
    ...
    for i in range(grid_size):
        for j in range(grid_size):
            ...
            values = np.sin(np.linspace(0, 4*np.pi, 100) + i * np.pi / 5) * 5 + \
                    np.cos(np.linspace(0, 6*np.pi, 100) + j * np.pi / 5) * 3 + 15
...
def main():
    ...
    reduced_data = CoordinateCollection(
        step=step_50.step,
        columns={name: step_50.columns[name].clone() for name in sensor_names if name in step_50.columns},
    ...
    try:
        ...
        print("scikit-learnがインストールされていないため、クラスタリングをスキップします")
