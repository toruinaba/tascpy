...
import pytest
import numpy as np
from src.tascpy.operations.core.column_combine import (
    column_switch_by_step,
    column_blend_by_step,
    column_conditional_select,
    column_custom_combine,
)
...
class TestColumnSwitchByStep:
    """column_switch_by_step関数のテスト"""

    def test_switch_by_index(self, sample_collection):
        """インデックス基準の切り替えテスト"""
        result = column_switch_by_step(
            sample_collection,
            "A",
            "B",
    ...
    def test_invalid_column(self, sample_collection):
        ...
        with pytest.raises(KeyError):
            column_switch_by_step(sample_collection, "NonExistent", "B", 2)

        with pytest.raises(KeyError):
            column_switch_by_step(sample_collection, "A", "NonExistent", 2)


class TestColumnBlendByStep:
    """column_blend_by_step関数のテスト"""

    def test_linear_blend_by_index(self, sample_collection):
        """インデックス基準の線形ブレンドテスト"""
        result = column_blend_by_step(
            sample_collection,
            "A",
            "B",
        ...
        expected = [
            1.0,  # インデックス0: A
            2.0 * 0.0 + 20.0 * 1.0,  # インデックス1: 完全にB (t=0)
            3.0 * 0.5 + 30.0 * 0.5,  # インデックス2: A:Bを半々で混合 (t=0.5)
            4.0 * 0.0 + 40.0 * 1.0,  # インデックス3: 完全にB (t=1)
            50.0,  # インデックス4: B
        ]
    ...
    def test_smooth_blend(self, sample_collection):
        ...
        assert values[1] > 2.0 and values[1] < 20.0
        assert values[2] > 3.0 and values[2] < 30.0
        assert values[3] > 4.0 and values[3] < 40.0
...
class TestColumnConditionalSelect:
    """column_conditional_select関数のテスト"""

    def test_greater_than_condition(self, sample_collection):
        """「より大きい」条件のテスト"""
        result = column_conditional_select(
...
class TestColumnCustomCombine:
    """column_custom_combine関数のテスト"""

    def test_custom_add_function(self, sample_collection):
        """カスタム加算関数のテスト"""
        result = column_custom_combine(
            sample_collection,
            "A",
            "B",
