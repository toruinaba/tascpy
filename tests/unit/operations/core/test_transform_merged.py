...
class TestTrigonometricFunctions:
    ...
    def test_sin_radians(self, sample_collection, epsilon):
        ...
        # 浮動小数点の比較は許容誤差を考慮
        # 浮動小数点の比較は許容誤差を考慮
        np.testing.assert_allclose(result["sin(angle_rad)"].values, expected, atol=epsilon)
    ...
    def test_sin_degrees(self, sample_collection, epsilon):
        ...
        # 浮動小数点の比較は許容誤差を考慮
        # 浮動小数点の比較は許容誤差を考慮
        np.testing.assert_allclose(result["sin(angle_deg)"].values, expected, atol=epsilon)
    ...
    def test_cos_radians(self, sample_collection, epsilon):
        ...
        # 浮動小数点の比較は許容誤差を考慮
        # 浮動小数点の比較は許容誤差を考慮
        np.testing.assert_allclose(result["cos(angle_rad)"].values, expected, atol=epsilon)
    ...
    def test_cos_degrees(self, sample_collection, epsilon):
        ...
        # 浮動小数点の比較は許容誤差を考慮
        # 浮動小数点の比較は許容誤差を考慮
        np.testing.assert_allclose(result["cos(angle_deg)"].values, expected, atol=epsilon)
    ...
    def test_tan_radians(self, sample_collection, epsilon):
        ...
        # 最初の4つの要素を比較
        # 最初の4つの要素を比較
        np.testing.assert_allclose(result["tan(angle_rad)"].values[:4], expected, atol=epsilon)
    ...
    def test_trig_with_none(self, sample_collection):
        ...
        target = result["sin(with_none)"].values
        expected_arr = np.array([math.sin(1.0), np.nan, math.sin(3.0), np.nan, math.sin(5.0)])
        np.testing.assert_allclose(target, expected_arr, atol=1e-10, equal_nan=True)
...
class TestExponentialAndLogarithmicFunctions:
    ...
    def test_exp(self, sample_collection, epsilon):
        ...
        # 浮動小数点の比較は許容誤差を考慮
        # 浮動小数点の比較は許容誤差を考慮
        np.testing.assert_allclose(result["exp(exp_input)"].values, expected, atol=epsilon)
    ...
    def test_log_natural(self, sample_collection, epsilon):
        ...
        # 浮動小数点の比較は許容誤差を考慮
        # 浮動小数点の比較は許容誤差を考慮
        np.testing.assert_allclose(result["log(log_input)"].values, expected, atol=epsilon)
    ...
    def test_log_base10(self, sample_collection, epsilon):
        ...
        # 浮動小数点の比較は許容誤差を考慮
        # 浮動小数点の比較は許容誤差を考慮
        np.testing.assert_allclose(result["log10(log_input)"].values, expected, atol=epsilon)
    ...
    def test_log_custom_base(self, sample_collection, epsilon):
        ...
        # 浮動小数点の比較は許容誤差を考慮
        # 浮動小数点の比較は許容誤差を考慮
        np.testing.assert_allclose(result[f"log{base}(log_input)"].values, expected, atol=epsilon)
    ...
    def test_log_negative(self, sample_collection):
        ...
        expected_arr = np.array([np.nan, np.nan, np.nan, 0, math.log(4)])
        np.testing.assert_allclose(result["log(negative)"].values, expected_arr, atol=1e-10, equal_nan=True)
...
class TestPowerFunctions:
    ...
    def test_sqrt(self, sample_collection):
        ...
        np.testing.assert_array_equal(result["sqrt(sqrt_input)"].values, expected)
    ...
    def test_sqrt_negative(self, sample_collection):
        ...
        # 負の値の平方根はNoneになる
        expected_arr = np.array([np.nan, np.nan, 0.0, 1.0, 2.0])
        np.testing.assert_array_equal(result["sqrt(negative)"].values, expected_arr)
    ...
    def test_pow(self, sample_collection):
        ...
        np.testing.assert_array_equal(result[f"exp_input^{exponent}"].values, expected)
...
class TestOtherFunctions:
    ...
    def test_abs(self, sample_collection):
        ...
        np.testing.assert_array_equal(result["abs(abs_input)"].values, expected)
    ...
    def test_round_values(self):
        ...
        np.testing.assert_array_equal(result0["round(pi, 0)"].values, expected0)
        
        # 小数点以下2桁
        result2 = round_values(collection, "pi", decimals=2)
        assert "round(pi, 2)" in result2.columns
        expected2 = [3.14, 3.14, 3.14, 3.14, 3.14]
        np.testing.assert_array_equal(result2["round(pi, 2)"].values, expected2)
    ...
    def test_with_none(self, sample_collection):
        ...
        expected_abs = [1.0, np.nan, 3.0, np.nan, 5.0]
        np.testing.assert_array_equal(result_abs["abs(with_none)"].values, expected_abs)
        
        # round関数
        result_round = round_values(sample_collection, "with_none", decimals=1)
        assert "round(with_none, 1)" in result_round.columns
        expected_round = [1.0, np.nan, 3.0, np.nan, 5.0]
        np.testing.assert_array_equal(result_round["round(with_none, 1)"].values, expected_round)
...
class TestNormalizationFunctions:
    ...
    def test_normalize_minmax(self, sample_collection, epsilon):
        ...
        # 浮動小数点の比較は許容誤差を考慮
        # 浮動小数点の比較は許容誤差を考慮
        np.testing.assert_allclose(result["norm_minmax(norm_input)"].values, expected, atol=epsilon)
    ...
    def test_normalize_zscore(self, sample_collection, epsilon):
        ...
        # 浮動小数点の比較は許容誤差を考慮
        # 浮動小数点の比較は許容誤差を考慮
        np.testing.assert_allclose(result["norm_zscore(norm_input)"].values, expected, atol=epsilon)
    ...
    def test_normalize_constant_values(self, epsilon):
        ...
        np.testing.assert_allclose(result_minmax["norm_minmax(const)"].values, expected_minmax, atol=epsilon)
        ...
        np.testing.assert_allclose(result_zscore["norm_zscore(const)"].values, expected_zscore, atol=epsilon)
    ...
    def test_normalize_with_none(self, sample_collection, epsilon):
        ...
        expected_arr = np.array([0.0, np.nan, 0.5, np.nan, 1.0])
        np.testing.assert_allclose(result["norm_minmax(with_none)"].values, expected_arr, atol=epsilon, equal_nan=True)
...
class TestOperationChaining:
    ...
    def test_chained_operations(self, ops, epsilon):
        ...
        # 浮動小数点の比較は許容誤差を考慮
        # 浮動小数点の比較は許容誤差を考慮
        np.testing.assert_allclose(result["sin(angle_deg)"].values, expected_sin, atol=epsilon)
        
        np.testing.assert_allclose(result["abs(sin(angle_deg))"].values, expected_abs, atol=epsilon)
    ...
    def test_complex_transform_chain(self, ops, epsilon):
        ...
        # 浮動小数点の比較は許容誤差を考慮
        # 浮動小数点の比較は許容誤差を考慮
        np.testing.assert_allclose(result["norm_minmax(norm_input)^2"].values, expected_pow, atol=epsilon)
        
        np.testing.assert_allclose(result["cos(norm_minmax(norm_input)^2)"].values, expected_cos, atol=epsilon)