...
def generate_collection_list_operations_stub(stub_dir: Path) -> None:
    ...
    with list_proxy_file.open("w", encoding="utf-8") as f:
                    ...
                    if "tascpy.domains" in module_path:
                        ...
                        if len(parts) > 1:
                            ...
                            f.write(f"from {relative_path} import {class_name}\n")
                             ...
                             f.write(f"from tascpy.domains.{domain} import {class_name}\n")
                    else:
                        f.write(f"from tascpy.domains.{domain} import {class_name}\n")
        ...
        f.write("            **kwargs: 変換に渡す追加引数。ドメインごとに以下の引数が利用可能です。\n")
        f.write("                - strain: rosettes (Dict), rosette_metadata_key (str)\n")
        f.write("                - load_displacement: load_column (str), displacement_column (str)\n")
        f.write("                - coordinate: coordinates (Dict), coordinate_metadata_key (str)\n")
        f.write("                - timeseries: start_date (str/datetime), frequency (str)\n")
        f.write("                - signal: sample_rate (float)\n")
        f.write("            \n")
        f.write("        Returns:\n")
    ...
    for domain in domains:
        ...
        if domain == "core":
            with domain_file.open("a", encoding="utf-8") as f:
                ...
                f.write(f"            **kwargs: 変換に渡す追加の引数。ドメインごとに以下の引数が利用可能です。\n")
                f.write(f"                - strain: rosettes (Dict), rosette_metadata_key (str)\n")
                f.write(f"                - load_displacement: load_column (str), displacement_column (str)\n")
                f.write(f"                - coordinate: coordinates (Dict), coordinate_metadata_key (str)\n")
                f.write(f"                - timeseries: start_date (str/datetime), frequency (str)\n")
                f.write(f"                - signal: sample_rate (float)\n")
                f.write(f"        \n")
                f.write(f"        Returns:\n")
                f.write(
